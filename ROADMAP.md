...
| `context_switch` | sched | `ContextSwitch` | Raw AArch64 register-file switching: `TaskContext` (x19–x28, fp, lr, sp, ttbr0), naked-asm `switch_to`/`jump_to`. `init_kernel_context` / `init_user_context` (drops to EL0) / `init_forked_context` (resumes a forked child at EL0 from the parent's trap frame with x0 = 0). `switch_to` saves/restores TTBR0. |
