...
VOID
Jump_To_Kernel(EFI_VIRTUAL_ADDRESS Entry, EFI_VIRTUAL_ADDRESS BootInfoPtr, EFI_VIRTUAL_ADDRESS Stack)
{
    asm volatile (
        "mov x0, %0\n\t"      // First arg in x0
        "mov sp, %1\n\t"      // Set stack pointer
...
efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    Status = SystemTable->BootServices->AllocatePool(EfiRuntimeServicesCode, sizeof(BootInfoStruct), (VOID**)&BootInfo);
    if (EFI_ERROR(Status))
    {
    ...
    Status = SystemTable->BootServices->AllocatePages(AllocateAnyPages, EfiRuntimeServicesCode, STACK_SIZE_PAGES, &StackPhysicalAddress);
    if (EFI_ERROR(Status))
    {
    ...
    Status = Map_Memory(SystemTable, &UpperPageTable, 0xFFFF800000000000, StackPhysicalAddress, 0, STACK_SIZE_PAGES);
    if (EFI_ERROR(Status))
    {
    ...
    Jump_To_Kernel(Entry, 0, 0xFFFF800000000000 + (4096 * STACK_SIZE_PAGES));
