...
EFI_STATUS
Enable_Page_Table(IN PAGE_TABLE_T LowerPageTable,
...
EFI_STATUS
Map_Memory(IN EFI_SYSTEM_TABLE *SystemTable, IN PAGE_TABLE_T *PageTable,
	   IN EFI_VIRTUAL_ADDRESS VirtualAddress,
	   IN EFI_PHYSICAL_ADDRESS PhysicalAddress, IN UINTN PageOrder,
	   IN UINTN PageCount)
{
	EFI_STATUS Status;
	...
	for (UINT64 I = 0; I < PageCount; I++) {
		EFI_VIRTUAL_ADDRESS CurrentVirtualAddress =
			VirtualAddress + I * Page_Order_Size(PageOrder);
		EFI_PHYSICAL_ADDRESS CurrentPhysicalAddress =
			PhysicalAddress + I * Page_Order_Size(PageOrder);
		...
		// 4KB pages (order 0)
		if (PageOrder == 0) {
			P3[Idx.P3] = CurrentPhysicalAddress |
				     ARM_4KB_PAGE_FLAGS;
			continue;
		}
	}
			   ...
			   IN UINTN TotalMemory, OUT PAGE_TABLE_T *PageTable)
{
	...
	if (BlocksNeeded == 0)
		BlocksNeeded = 1; // At least one block
