...
const ConfigMod = @import("module_config.zig");
...
pub const ModuleLoader = struct {
    ...
    fn loadModule(
        ...
        configs: *ConfigMod.ModuleConfigHandles,
    ) !void {
        ...
        try self.loadElf(buf, name, page_table, registry, imports, configs);
    }
    ...
    fn loadElf(
        ...
        registry.registerModuleOwner(module_name, owner_type, owner_name) catch |err| {
            Serial.failLog("Registered module owner");
            return err;
        };
        Serial.okLog("Registered module exports");

        // 4. Queue every `.kmodule.import.<type>` placeholder for later
        //    resolution against the [Dependencies] graph (once the MMU is
        //    enabled).
        for (shdrs) |shdr| {
            const name = sectionName(buf, shstrtab, shdr);
            const type_name = stripPrefix(name, shared.IMPORT_SECTION_PREFIX) orelse continue;
        ...
        // 5. Queue every `.kmodule.config.<kind>.<key>` placeholder for
        //    later resolution against kernel.ini's `[module.config]` values
        //    (once the MMU is enabled -- see module_config.zig).
        for (shdrs) |shdr| {
            const name = sectionName(buf, shstrtab, shdr);
            const rest = stripPrefix(name, shared.CONFIG_SECTION_PREFIX) orelse continue;
            const dot = std.mem.indexOfScalar(u8, rest, '.') orelse {
                Serial.failLog("Parsed config section name");
                return error.InvalidConfigSection;
            };
            const kind = rest[0..dot];
            const key = rest[dot + 1 ..];
            ...
            configs.add(.{
                ...
                Serial.failLog("Queued module config");
                return err;
            };
        }
        Serial.okLog("Queued module config");
    }

    /// A `[module_name]` section (module declaration -- expects an `enable
    /// = true/false` line), a `[module_name.dependencies]` section (expects
    /// `type = provider_module` lines), or a `[module_name.config]` section
    /// (expects `key = value` lines). All three carry the owning module's
    /// name, recovered by stripping the `.dependencies`/`.config` suffix
    /// for the latter two.
    ...
    /// Parses kernel.ini into: an ordered list of modules to load (every
    /// `enable = true` module, in file order), the dependency graph, and
    /// every module's raw `[module.config]` values -- but does NOT load any
    /// module yet. Loading is deferred to `loadKernel` (see its doc-comment
    /// for why): a `[module.config]` section can appear anywhere relative
    /// to `[module_name]` in the file, and `loadElf` needs those values
    /// already collected before it scans a module's config placeholders.
    fn parseConfig(
        allocator: std.mem.Allocator,
        graph: *DependencyGraphMod.DependencyGraph,
        values: *ConfigMod.ConfigValues,
        queued: *std.ArrayList([]const u8),
        config_buffer: []u8,
    ) !void {
        // `config_buffer` is NUL-terminated (see `Filesystem.readFile`) --
        // that trailing NUL is not file content, and left in would show up
        // as a phantom last "line" consisting of just the NUL byte.
        var line_iter = std.mem.splitScalar(u8, std.mem.sliceTo(config_buffer, 0), '\n');
        ...
        while (line_iter.next()) |raw_line| {
            ...
            if (line[0] == '[') {
                ...
                if (std.mem.endsWith(u8, name, DEPENDENCIES_SUFFIX)) {
                    section = .{ .dependencies = name[0 .. name.len - DEPENDENCIES_SUFFIX.len] };
                } else if (std.mem.endsWith(u8, name, CONFIG_SUFFIX)) {
                    section = .{ .config = name[0 .. name.len - CONFIG_SUFFIX.len] };
                } else {
                    section = .{ .module = name };
                }
                continue;
            ...
            switch (section) {
                ...
                .module => |module_name| {
                    ...
                    const raw_key = kv_iter.next() orelse continue;
                    const raw_value = kv_iter.next() orelse continue;

                    const key = std.mem.trim(u8, raw_key, " \t");
                    if (!std.mem.eql(u8, key, "enable")) continue;

                    const value = std.mem.trim(u8, raw_value, " \t");
                    if (value.len > 0 and (value[0] == 't' or value[0] == 'T' or value[0] == '1')) {
                        queued.append(allocator, module_name) catch |err| {
                            Serial.failLog("Queued module load");
                            return err;
                        };
                    }
                },
                .dependencies => |module_name| {
                    var kv_iter = std.mem.splitScalar(u8, line, '=');
                    const raw_type = kv_iter.next() orelse continue;
                    const raw_provider = kv_iter.next() orelse continue;

                    const type_name = std.mem.trim(u8, raw_type, " \t");
                    const provider_module = std.mem.trim(u8, raw_provider, " \t");

                    graph.addEdge(module_name, type_name, provider_module) catch |err| {
                        Serial.failLog("Parsed module dependency line");
                        return err;
                    };
                },
                .config => |module_name| {
                    ...
                    values.add(module_name, key, value) catch |err| {
                        Serial.failLog("Parsed module config line");
                        return err;
                    };
                },
            }
    ...
    pub fn loadKernel(
        ...
        try parseConfig(allocator, graph, values, &queued, config_buffer);

        // Load every enabled module now that the full file has been
        // scanned (and every `[module.config]` value collected), in the
        // same order their `[module_name]` sections were encountered.
        for (queued.items) |module_name| {
            try self.loadModule(root, module_name, page_table, registry, imports, configs);
        }
    }
};
