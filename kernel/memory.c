...
{
	serial_debug_serial_printf("huh %lx %ld\n", bootInfo->memoryRegions,
				   bootInfo->memoryMapSize);
	for (int i = 0; i < bootInfo->memoryMapSize; ++i) {
		if (bootInfo->memoryRegions[i].page_count >= KERNEL_SLAB_SIZE &&
		    bootInfo->memoryRegions[i].type == MEMORY_FREE) {
			kernel_slab = (void *)bootInfo->memoryRegions[i].start;
			bootInfo->memoryRegions[i].page_count -=
				KERNEL_SLAB_SIZE;
			bootInfo->memoryRegions[i].start +=
				(KERNEL_SLAB_SIZE * PAGE_SIZE);
		}
	}

	serial_debug_serial_printf("Allocated Something\n");

	if (!kernel_slab) {
		// panic
		serial_debug_serial_printf("uh oh\n");
	}
	...
	*kernel_slab = 0;
	...
	for (int i = 1; i < KERNEL_SLAB_SIZE; ++i) {
		unsigned long *page =
			(void *)((unsigned long)kernel_slab + i * PAGE_SIZE);
