...
k_status_t k_slab_destroy_cache(k_slab_cache_t *cache) {
    ...
    root     = vmm_get_kernel_space_root();
    ...
    for (i = 0; i < 3; i++) {
        ...
        while (curr) {
            next = curr->next;
            vmm.free(root, (vaddr_t)curr->page_base, SLAB_PAGE_SIZE);
            free_slab_descriptor(curr);
            curr = next;
        }
    }

    memset(cache, 0, sizeof(k_slab_cache_t));
...
k_status_t k_slab_alloc(k_slab_cache_t *cache, void **out_obj) {
    ...
    if (cache->slabs_partial) {
        ...
        root   = vmm_get_kernel_space_root();
        status = vmm.allocate(root, &vaddr, SLAB_PAGE_SIZE, MMU_READ | MMU_WRITE, VMM_REGION_HEAP);
        if (k_error(status)) {
            goto cleanup;
        }
        ...
        if (!s) {
            vmm.free(root, vaddr, SLAB_PAGE_SIZE);
            status = K_STATUS_OUT_OF_MEMORY;
            goto cleanup;
        }
        ...
        for (i = 0; i < cache->slots_per_slab - 1; i++) {
            uint32_t *next_link = (uint32_t *)(base + (i * cache->obj_size));
            *next_link = i + 1;
        }
        uint32_t *last_link = (uint32_t *)(base + ((cache->slots_per_slab - 1) * cache->obj_size));
    ...
    alloc_addr        = (uintptr_t)s->page_base + (s->next_free_slot * cache->obj_size);
