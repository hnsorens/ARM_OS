...
const mmu_if = abi.importInterface(abi.Mmu);
...
pub const VMM_DEFAULT_ALIGNMENT: u64 = 4096;
...
pub fn spaceCreate(out_table_root: *u64) callconv(.c) c_int {
    var root: u64 = 0;
...
pub fn spaceDestroy(table_root: u64) callconv(.c) c_int {
...
pub fn allocate(root: u64, vaddr: *u64, sz_in: u64, flags: u64, region_type: abi.RegionType) callconv(.c) c_int {
...
pub fn reserve(root: u64, vaddr: u64, sz_in: u64) callconv(.c) c_int {
...
pub fn free(root: u64, vaddr: u64, sz_in: u64) callconv(.c) c_int {
    const space = getSpace(root) orelse return abi.EINVAL;
...
pub fn resize(root: u64, vaddr: u64, old_sz: u64, new_sz_in: u64) callconv(.c) c_int {
    const space = getSpace(root) orelse return abi.EINVAL;
...
pub fn mapExternal(root: u64, v: u64, p: u64, sz_in: u64, f: u64) callconv(.c) c_int {
    const space = getSpace(root) orelse return abi.EINVAL;
    const sz = alignUp(sz_in);
...
pub fn protect(root: u64, vaddr: u64, sz_in: u64, new_flags: u64) callconv(.c) c_int {
    const space = getSpace(root) orelse return abi.EINVAL;
...
pub fn query(root: u64, vaddr: u64, out_info: *abi.VmmRegionInfo) callconv(.c) c_int {
    const space = getSpace(root) orelse return abi.EINVAL;
    const vma = findVma(space, vaddr) orelse return abi.EINVAL;
...
pub fn activate(root: u64) callconv(.c) c_int {
    return mmu_if.set_user_ctx(root, 1);
}

pub fn sync(root: u64, vaddr: u64, sz: u64) callconv(.c) c_int {
    _ = root;
