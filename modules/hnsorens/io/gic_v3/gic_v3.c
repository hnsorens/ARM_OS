...
int configure(irq_vector_t vector, irq_trigger_t trigger, irq_prio_t priority)
{
	...
	if (vector < 32) {
		...
		/* IGROUPR */
		uint32_t igroupr = mmio_read32(base + GICR_SGI_OFFSET + 0x0080 +
					       (grp_idx * 4));
		igroupr |= (1U << bit_shift);
		mmio_write32(base + GICR_SGI_OFFSET + 0x0080 + (grp_idx * 4),
			     igroupr);
		/* IGRPMODR – clear for Non‑Secure */
		uint32_t igrpmodr = mmio_read32(base + GICR_SGI_OFFSET +
						0x0D00 + (grp_idx * 4));
		igrpmodr &= ~(1U << bit_shift);
		mmio_write32(base + GICR_SGI_OFFSET + 0x0D00 + (grp_idx * 4),
			     igrpmodr);
	} else {
		base = g_gicd;
		/* IPRIORITYR */
