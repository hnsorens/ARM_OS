...
static uint32_t safe_mmio_read(uintptr_t addr, const char* reg_name) {
    DEBUG("Reading %s at 0x%08lx", reg_name, addr);
    uint32_t value = *(volatile uint32_t*)addr;
    DEBUG("  Value: 0x%08x", value);
    return value;
}

static void safe_mmio_write(uintptr_t addr, uint32_t value, const char* reg_name) {
    DEBUG("Writing %s at 0x%08lx: 0x%08x", reg_name, addr, value);
...
static uintptr_t find_virtio_device(uint8_t id) {
    ...
    for (uintptr_t base = 0x0A000000; base < 0x0A004000; base += 0x200) {
        ...
        uint32_t magic = safe_mmio_read(base, "Magic");
        
        if (magic == VIRTIO_MAGIC) {
            DEBUG("Found VirtIO device at 0x%08lx", base);
            
            uint32_t version = safe_mmio_read(base + 0x004, "Version");
            uint32_t device_id = safe_mmio_read(base + 0x008, "Device ID");
            uint32_t vendor_id = safe_mmio_read(base + 0x00C, "Vendor ID");
...
static int virtio_mmio_common_init(uintptr_t base) {
     ...
     * 0. Identify device
     * ------------------------------------------------------------ */
    uint32_t magic   = safe_mmio_read(base + 0x000, "MAGIC");
    uint32_t version = safe_mmio_read(base + 0x004, "VERSION");
    uint32_t device  = safe_mmio_read(base + 0x008, "DEVICE_ID");
    ...
    // Reset device
    safe_mmio_write(base + 0x070, 0x00, "STATUS reset");

    // ACKNOWLEDGE + DRIVER
    uint32_t status = 0;

    status |= 0x01; /* ACKNOWLEDGE */
    safe_mmio_write(base + 0x070, status, "STATUS |= ACK");

    status |= 0x02; /* DRIVER */
    safe_mmio_write(base + 0x070, status, "STATUS |= DRIVER");

    // Discover device features
    uint32_t dev_features[2] = {0, 0};

    for (uint32_t i = 0; i < 2; i++) {
        safe_mmio_write(base + 0x014, i, "DEVICE_FEATURES_SEL");
        dev_features[i] = safe_mmio_read(base + 0x010, "DEVICE_FEATURES");
    ...
    for (uint32_t i = 0; i < 2; i++) {
        safe_mmio_write(base + 0x024, i, "DRIVER_FEATURES_SEL");
        safe_mmio_write(base + 0x020, drv_features[i], "DRIVER_FEATURES");
    }

    // FEATURES_OK handshake
    status |= 0x08; /* FEATURES_OK */
    safe_mmio_write(base + 0x070, status, "STATUS |= FEATURES_OK");

    status = safe_mmio_read(base + 0x070, "STATUS readback");
    if (!(status & 0x08)) {
