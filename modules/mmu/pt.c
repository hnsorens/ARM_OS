...
static inline pte_t *l2v(paddr_t phys) {
    // Return a uniform pte_t* type to handle pointer offsets accurately
...
static pt_indices_t extract_indices(vaddr_t virt) {
    ...
    idx.l3_index = P3_INDEX(virt);   /* Page Table index */
    idx.l2_index = P2_INDEX(virt);   /* Page Directory index */
    idx.l1_index = P1_INDEX(virt);   /* PDPT index */
    idx.l0_index = P0_INDEX(virt);   /* PML4 index */
    return idx;
}
...
static void pte_recursive_free(paddr_t pt_phys, int level) {
    ...
    // Free the current table frame page
...
k_status_t pt_free(paddr_t root) {
    ...
    // Start recursive destruction from Level 0
...
k_status_t pt_copy(paddr_t src_root, paddr_t *dst_root) {
    if (!src_root || !dst_root) return K_STATUS_BAD_PHYS_ADDR;
...
k_status_t pt_set_kernel_ctx(paddr_t root, asid_t asid) {
    // AArch64 ASID lives in bits [63:48] of TTBR1_EL1
    uint64_t ttbr = ((uint64_t)asid << 48) | (root & PAGE_MASK);
...
k_status_t pt_map(paddr_t root, vaddr_t virt, paddr_t phys, uint64_t pg_count, page_size_t pg_size, mmu_flags_t f) {
    ...
    // FIXED: Upgraded loop counter to uint64_t to match page count constraints
    for (uint64_t i = 0; i < pg_count; i++) {
        ...
        if (!(l0_pte & 1)) { 
            paddr_t l1_phys;
            if (k_error(pmm.alloc_page(0, &l1_phys))) return K_STATUS_OUT_OF_MEMORY;
        ...
        if (!(l1_pte & 1)) {
            paddr_t l2_phys;
            if (k_error(pmm.alloc_page(0, &l2_phys))) return K_STATUS_OUT_OF_MEMORY;
        ...
        if (!(l2_pte & 1)) {
            paddr_t l3_phys;
            if (k_error(pmm.alloc_page(0, &l3_phys))) return K_STATUS_OUT_OF_MEMORY;
    ...
    for (size_t i = 0; i < pg_count; i++) {
        ...
        if (pg_size == PS_1GB) {
            ...
            if (pg_size == PS_2MB) {
                ...
                // PRUNING: Verify and release page table if entry block is clear
...
k_status_t pt_protect(paddr_t root, vaddr_t virt, uint64_t pg_count, page_size_t pg_size, mmu_flags_t f) {
    ...
    for (size_t i = 0; i < pg_count; i++) {
        ...
        if (!(l0[idx.l0_index] & 1)) return K_STATUS_NOT_MAPPED;
        pte_t *l1 = l2v(l0[idx.l0_index] & PAGE_MASK);

        if (pg_size == PS_1GB) {
            if (!(l1[idx.l1_index] & 1)) return K_STATUS_NOT_MAPPED;
        ...
        if (pg_size == PS_2MB) {
            if (!(l2[idx.l2_index] & 1)) return K_STATUS_NOT_MAPPED;
        ...
        if (pg_size == PS_4KB) {
            if (!(l3[idx.l3_index] & 1)) return K_STATUS_NOT_MAPPED;
...
k_status_t pt_translate(paddr_t root, vaddr_t virt, paddr_t *phys_out, mmu_flags_t *flags_out) {
    if (!root || !phys_out || !flags_out) return K_STATUS_BAD_PHYS_ADDR;
...
k_status_t pt_invalidate(vaddr_t virt, uint64_t pg_count, page_size_t pg_size) {
    for (size_t i = 0; i < pg_count; ++i) {
        ...
        // FIXED: Replaced raw uint64_t translation with an exact vaddr_t width element
