...
void pmm_init(memory_region_t *memory_map, size_t region_count, paddr_t hhdm_offset) {
    ...
    size_t meta_pages_needed = (meta_array_size + PMM_PAGE_SIZE - 1) / PMM_PAGE_SIZE;
    ...
    for (size_t i = 0; i < region_count; i++) {
        if (memory_map[i].memory_type == MEMORY_FREE && 
            ...
            memory_map[i].start += (meta_pages_needed * PMM_PAGE_SIZE);
            memory_map[i].size  -= (meta_pages_needed * PMM_PAGE_SIZE);
            break;
        }
    }

    g_pmm_meta_array = (pmm_page_meta_t *)paddr_to_kv(meta_phys_alloc_start);
    ...
    for (size_t i = 0; i < g_pmm_total_pages; i++) {
        ...
        g_pmm_meta_array[i].is_free   = 0; /* Default block behavior: Marked Allocated */
    }
    ...
    for (size_t i = 0; i < region_count; i++) {
        if (memory_map[i].memory_type != MEMORY_FREE) continue;
        ...
        while (chunk_cursor < chunk_end) {
            ...
            while (target_order > 0) {
                size_t block_bytes = (1UL << target_order) * PMM_PAGE_SIZE;
                if (block_bytes <= remaining_bytes && (chunk_cursor % block_bytes) == 0) {
                    break;
                }
                target_order--;
            }
            ...
            pmm_block_node_t *node = (pmm_block_node_t *)paddr_to_kv(chunk_cursor);
            pmm_list_add(target_order, node);

            size_t allocated_bytes = (1UL << target_order) * PMM_PAGE_SIZE;
...
k_status_t pmm_alloc_page(uint8_t page_order, paddr_t *out_frame) {
    if (page_order >= PMM_MAX_ORDER || !out_frame) return K_STATUS_INVALID_ARG;

    for (uint8_t current_order = page_order; current_order < PMM_MAX_ORDER; current_order++) {
        ...
        while (current_order > page_order) {
            current_order--;
            size_t split_block_size = (1UL << current_order) * PMM_PAGE_SIZE;
            paddr_t buddy_phys_part = found_block_phys + split_block_size;
            size_t buddy_index_part = paddr_to_index(buddy_phys_part);

            g_pmm_meta_array[buddy_index_part].is_free = 1;
            g_pmm_meta_array[buddy_index_part].order   = current_order;
            g_pmm_meta_array[buddy_index_part].ref_count = 0;

            pmm_block_node_t *buddy_node = (pmm_block_node_t *)paddr_to_kv(buddy_phys_part);
        ...
        g_pmm_allocator.free_memory_bytes -= (1UL << page_order) * PMM_PAGE_SIZE;
...
k_status_t pmm_free_page(uint8_t page_order, paddr_t frame) {
    if (page_order >= PMM_MAX_ORDER || (frame % PMM_PAGE_SIZE) != 0) return K_STATUS_INVALID_ARG;
    ...
    while (current_order < PMM_MAX_ORDER - 1) {
        ...
        if (buddy_index >= g_pmm_total_pages) break;
        ...
        pmm_block_node_t *buddy_node = (pmm_block_node_t *)paddr_to_kv(buddy_frame);
    ...
    pmm_block_node_t *final_node = (pmm_block_node_t *)paddr_to_kv(current_frame);
...
k_status_t pmm_alloc_aligned(size_t count, size_t alignment, paddr_t *out) {
    if (count == 0 || alignment < PMM_PAGE_SIZE || !out) return K_STATUS_INVALID_ARG;

    /* Calculate the order required to cleanly cover the requested page count */
    uint8_t target_order = 0;
    while ((1UL << target_order) < count) {
        target_order++;
        if (target_order >= PMM_MAX_ORDER) return K_STATUS_INVALID_ARG;
    }
        ...
        while (curr) {
            ...
            if ((phys % alignment) == 0) {
                ...
                while (active_order > target_order) {
                    active_order--;
                    size_t split_sz = (1UL << active_order) * PMM_PAGE_SIZE;
                    paddr_t split_buddy = phys + split_sz;
                    size_t buddy_idx = paddr_to_index(split_buddy);

                    g_pmm_meta_array[buddy_idx].is_free   = 1;
                    g_pmm_meta_array[buddy_idx].order     = active_order;
                    g_pmm_meta_array[buddy_idx].ref_count = 0;

                    pmm_list_add(active_order, (pmm_block_node_t *)paddr_to_kv(split_buddy));
                }

                g_pmm_meta_array[base_idx].order     = target_order;
                g_pmm_meta_array[base_idx].ref_count = 1;
                g_pmm_allocator.free_memory_bytes -= (1UL << target_order) * PMM_PAGE_SIZE;
...
k_status_t pmm_alloc_in_range(size_t count, paddr_t max_addr, paddr_t *out) {
    if (count == 0 || !out) return K_STATUS_INVALID_ARG;
    ...
    for (uint8_t o = target_order; o < PMM_MAX_ORDER; o++) {
        ...
        while (curr) {
            paddr_t phys = kv_to_paddr(curr);
            size_t allocation_bytes = (1UL << target_order) * PMM_PAGE_SIZE;
            ...
            if ((phys + allocation_bytes) <= max_addr) {
                ...
                g_pmm_meta_array[base_idx].order     = target_order;
                g_pmm_meta_array[base_idx].ref_count = 1;
                g_pmm_allocator.free_memory_bytes -= allocation_bytes;
...
void pmm_retain(paddr_t frame) {
    ...
    if (page_idx >= g_pmm_total_pages || g_pmm_meta_array[page_idx].is_free) return;
...
k_status_t pmm_reserve_range(paddr_t start, size_t sz) {
    if ((start % PMM_PAGE_SIZE) != 0 || sz == 0) return K_STATUS_INVALID_ARG;
    ...
    if (end_page_idx > g_pmm_total_pages) return K_STATUS_INVALID_ARG;
    ...
    for (size_t idx = start_page_idx; idx < end_page_idx; idx++) {
        if (g_pmm_meta_array[idx].is_free) {
            ...
            pmm_block_node_t *node = (pmm_block_node_t *)paddr_to_kv(block_base_phys);
            ...
            g_pmm_allocator.free_memory_bytes -= (1UL << current_order) * PMM_PAGE_SIZE;
        }
    }
