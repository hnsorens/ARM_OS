...
static void insert_vma(vmm_space_t *space, vm_area_t *vma) {
    ...
    while (curr) {
        if (vma->base < curr->base) {
            vma->next = curr;
            vma->prev = curr->prev;
            if (curr->prev) curr->prev->next = vma;
            else space->vma_head = vma;
            curr->prev = vma;
            return;
        }
        if (!curr->next) {
...
static vaddr_t find_unmapped_area(vmm_space_t *space, vaddr_t hint, size_t sz) {
    ...
    addr = (addr + (VMM_DEFAULT_ALIGNMENT - 1)) & ~(VMM_DEFAULT_ALIGNMENT - 1);
    ...
    while (addr + sz <= VMM_USER_SPACE_MAX) {
        ...
        while (curr) {
            if (!(addr + sz <= curr->base || addr >= curr->base + curr->size)) {
                conflict = curr;
                break;
            }
            curr = curr->next;
        ...
        addr = (conflict->base + conflict->size + (VMM_DEFAULT_ALIGNMENT - 1)) & ~(VMM_DEFAULT_ALIGNMENT - 1);
    }
    return 0;
}
...
k_status_t vmm_space_create(paddr_t *out_table_root) {
    ...
    if (!out_table_root) return K_STATUS_INVALID_ARG;
...
k_status_t vmm_space_destroy(paddr_t table_root) {
    ...
    if (!space) return K_STATUS_NOT_FOUND;

    vm_area_t *curr = space->vma_head;
    while (curr) {
        vm_area_t *next = curr->next;
        if (curr->is_paged) {
            mmu.unmap(table_root, curr->base, curr->size / VMM_DEFAULT_ALIGNMENT, PS_4KB);
        }
        vma_free(curr);
        curr = next;
    }
...
k_status_t vmm_allocate(paddr_t root, vaddr_t *vaddr, size_t sz, mmu_flags_t flags, vmm_region_type_t type) {
    ...
    if (!root || !vaddr || sz == 0) return K_STATUS_INVALID_ARG;
    ...
    vma = vma_alloc();
    if (!vma) {
        if (guard_offset > 0) vmm_free(root, target_addr, VMM_DEFAULT_ALIGNMENT);
        return K_STATUS_OUT_OF_MEMORY;
    }
    ...
    for (i = 0; i < sz; i += VMM_DEFAULT_ALIGNMENT) {
        ...
        if (k_error(status)) goto cleanup;

        status = mmu.map(root, vma->base + i, phys_page, 1, PS_4KB, flags);
        if (k_error(status)) {
...
k_status_t vmm_reserve(paddr_t root, vaddr_t vaddr, size_t sz) {
    ...
    if (vmm_query(root, vaddr, &info) == K_STATUS_OK) return K_STATUS_ALREADY_EXISTS;

    vma = vma_alloc();
    if (!vma) return K_STATUS_OUT_OF_MEMORY;
...
k_status_t vmm_free(paddr_t root, vaddr_t vaddr, size_t sz) {
    ...
    if (!vma || vaddr < vma->base || (vaddr + sz) > (vma->base + vma->size)) return K_STATUS_NOT_FOUND;
    ...
    if (vma->is_paged) {
        for (size_t i = 0; i < sz; i += VMM_DEFAULT_ALIGNMENT) {
            ...
            if (mmu.translate(root, vaddr + i, &phys, &f) == K_STATUS_OK) {
    ...
    if (vaddr == vma->base && sz == vma->size) {
        ...
        if (space->vma_head == vma) space->vma_head = vma->next;
        if (space->mmap_cache == vma) space->mmap_cache = NULL;
        vma_free(vma);
        ...
        if (!split_vma) return K_STATUS_OUT_OF_MEMORY;
...
k_status_t vmm_resize(paddr_t root, vaddr_t vaddr, size_t old_sz, size_t new_sz) {
    ...
    if (!vma || vma->base != vaddr || vma->size != old_sz) return K_STATUS_NOT_FOUND;

    new_sz = (new_sz + (VMM_DEFAULT_ALIGNMENT - 1)) & ~(VMM_DEFAULT_ALIGNMENT - 1);
    if (new_sz == old_sz) return K_STATUS_OK;
    ...
    if (vma->next && vma->next->base < (next_vaddr + growth)) return K_STATUS_OUT_OF_MEMORY;
    if (next_vaddr + growth > VMM_USER_SPACE_MAX) return K_STATUS_OUT_OF_MEMORY;

    /* Allocation Expansion Loop with explicit rollback safety */
    for (i = old_sz; i < new_sz; i += VMM_DEFAULT_ALIGNMENT) {
        paddr_t phys_page;
        status = pmm.alloc_page(0, &phys_page);
        if (k_error(status)) goto rollback_expansion;

        status = mmu.map(root, vaddr + i, phys_page, 1, PS_4KB, vma->flags);
        if (k_error(status)) {
            pmm.free_page(0, phys_page);
...
rollback_expansion:
    ...
    for (size_t undo = old_sz; undo < i; undo += VMM_DEFAULT_ALIGNMENT) {
        paddr_t     phys = 0;
        mmu_flags_t f    = 0;
        if (mmu.translate(root, vaddr + undo, &phys, &f) == K_STATUS_OK) {
...
k_status_t vmm_protect(paddr_t root, vaddr_t vaddr, size_t sz, mmu_flags_t new_flags) {
    ...
    status = mmu.protect(root, vaddr, sz / VMM_DEFAULT_ALIGNMENT, PS_4KB, new_flags);
    if (k_error(status)) return status;
...
k_status_t vmm_query(paddr_t root, vaddr_t vaddr, vmm_region_info_t *out_info) {
    ...
    if (!vma) return K_STATUS_NOT_MAPPED;
...
k_status_t vmm_sync(paddr_t root, vaddr_t vaddr, size_t sz) {
    (void)root;
    size_t pages = (sz + (VMM_DEFAULT_ALIGNMENT - 1)) / VMM_DEFAULT_ALIGNMENT;
